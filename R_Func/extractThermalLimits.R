...
extractThermalLimits <- function(
  ...
  configPath <- file.path(GlobSavingDir, folder, paste("FindMetrics", folder, sep = "_"), configName)
  ...
  if (interactive) {
    for (i in 1:1000) {
      ...
      plot(
        ...
        ylim = c(0, max(c(SmoothedResCIMetric$X2.5., SmoothedResCIMetric$SmoothedMean), na.rm = TRUE)),
      ...
      if (all(c("X2.5.", "X97.5.") %in% names(SmoothedResCIMetric))) {
        ...
        polygon(
          x = c(SmoothedResCIMetric$runTimelinef[valid], rev(SmoothedResCIMetric$runTimelinef[valid])),
          y = c(SmoothedResCIMetric$X2.5.[valid], rev(SmoothedResCIMetric$X97.5.[valid])),
      ...
      if (grepl("^na$", BP, ignore.case = TRUE)) break
      BP <- as.integer(BP)
      Segmod <- segmented::segmented(mod, seg.Z = ~x, npsi = BP)
      plot(Segmod, add = TRUE, col = "firebrick3", lwd = 2)
      
      satisfied <- readline("Is the result satisfactory? (Y/N): ")
      if (tolower(satisfied) == "y") break
    }
    ...
    Plateaux <- c(list(c(0, breakpoints$runTimelinef[1])),
                  lapply(seq_len(nrow(breakpoints)), function(j) {
                    if (j == nrow(breakpoints)) {
                      c(breakpoints$runTimelinef[j], max(SmoothedResCIMetric$runTimelinef))
                    } else {
                      c(breakpoints$runTimelinef[j], breakpoints$runTimelinef[j + 1])
                    }
                  }))
    print(Plateaux)
    ...
    userChoices[[strainName]][[folder]][[metricName]] <- list(
      nBreakpoints = BP,
  ...
  meanValue <- mean(SmoothedResCIMetric$SmoothedMean[
    SmoothedResCIMetric$runTimelinef >= 15 * 60 * frameRate &
